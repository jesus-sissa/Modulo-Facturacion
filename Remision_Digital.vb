...
#End Region
    ...
    Public Shared Function fn_ObtenTipoMoneda(ByVal IdMoneda As Integer) As String
        ...
        End Try
    End Function
    Shared Function QrMateriales(texto As String) As Image
        Dim Oqr As BarcodeQRCode = New BarcodeQRCode(texto, 1000, 1000, Nothing)
        Dim img As Image = Oqr.GetImage()
        img.ScaleAbsolute(60, 60)
        Return img
    End Function

    Public Shared Function CrearCeldaPdf(Columna As PdfPCell, Optional ByVal BaseColor As BaseColor = Nothing, Optional ByVal ColSpan As Integer = Nothing, Optional ByVal Alineacion As Integer = Nothing, Optional Border As Integer = 0) As PdfPCell
        Columna.Border = 0
        Columna.Colspan = ColSpan
        Columna.BackgroundColor = BaseColor
        Columna.HorizontalAlignment = Alineacion
        Return Columna
    End Function

    Shared Function Codigo_barrasMateriales(NumeroRemision) As Byte()
        Dim code As Barcode128 = New Barcode128()
        code.CodeType = Barcode128.CODE128
        code.Code = NumeroRemision
        code.AltText = NumeroRemision
    ...
    Public Shared Function PdfMateriales(dtMatNotificaciones As DataTable, envasesN As String, Path As String) As MemoryStream

        Dim ms As MemoryStream = New MemoryStream()
        Dim doc As New Document(PageSize.A4, 15.0F, 15.0F, 40.0F, 30.0F)

        'pdfw = PdfWriter.GetInstance(doc, ms)
        pdfw = PdfWriter.GetInstance(doc, New FileStream($"{Path}\{dtMatNotificaciones.Rows(0).Item("Numero_Remision")}.pdf", FileMode.Create))
        Dim fontN As New Font(FontFactory.GetFont(FontFactory.HELVETICA, 8, iTextSharp.text.Font.NORMAL))
        Dim fontB As New Font(FontFactory.GetFont(FontFactory.HELVETICA, 8, iTextSharp.text.Font.BOLD))
        Dim fontB12 As New Font(FontFactory.GetFont(FontFactory.HELVETICA, 12, iTextSharp.text.Font.BOLD))
        Dim cvacio As PdfPCell = New PdfPCell(New Phrase())
        ...
        doc.Open()
        Dim tabla As PdfPTable = New PdfPTable(4)
        Dim col1, col2, col3, col4 As PdfPCell
        ...
        tabla.AddCell(cvacio)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("Servicio Integral de Seguridad. S.A. de C.V.", fontB12)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)
        tabla.AddCell(cvacio)
        tabla.AddCell(cvacio)

        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("ALVAREZ NTE. 209 MONTERREY, N.L. C.P. 64000" + vbCrLf + "CONMUTADOR: 8047-4545, 8047-4546 FAX 8047 4550" + vbCrLf + "www.sissaseguridad.com", fontN)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)

        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("REMISION:" + vbCrLf + dtMatNotificaciones.Rows(0).Item("Numero_Remision").ToString() + vbCrLf, fontB)))
        tabla.AddCell(col3)
        tabla.AddCell(cvacio)

        col2 = CrearCeldaPdf(New PdfPCell(iTextSharp.text.Image.GetInstance(Codigo_barrasMateriales(dtMatNotificaciones.Rows(0).Item("Numero_Remision").ToString()))), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)

        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("Ruta:" + vbCrLf + dtMatNotificaciones.Rows(0).Item("Ruta").ToString() + vbCrLf + "Unidad:" + vbCr + dtMatNotificaciones.Rows(0).Item("Unidad").ToString(), fontN)))
        tabla.AddCell(col3)
        doc.Add(tabla)
        ...
        col1 = New PdfPCell(New Phrase(FormatNumber("0", 2), fontB))
        tabla2.AddCell(col1)
        ...
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("VALORES RECIBIDOS DE :", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Nombre_cia").ToString(), fontN)))
        tabla.AddCell(col2)
        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("MONEDA NACIONAL:", fontB)))
        tabla.AddCell(col3)
        col4 = CrearCeldaPdf(New PdfPCell(New Phrase(FormatNumber(dtMatNotificaciones.Rows(0).Item("ImporteTotal").ToString(), 2), fontN)))
        tabla.AddCell(col4)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("NUM. CLIENTE:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Clave_Cliente").ToString(), fontN)))
        tabla.AddCell(col2)
        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("MONEDA EXTRANJERA:", fontB)))
        tabla.AddCell(col3)
        col4 = CrearCeldaPdf(New PdfPCell(New Phrase("0.00", fontN)))
        tabla.AddCell(col4)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("FECHA:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Fecha_Valida").ToString(), fontN)))
        tabla.AddCell(col2)
        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("OTROS:", fontB)))
        tabla.AddCell(col3)
        col4 = CrearCeldaPdf(New PdfPCell(New Phrase("0.00", fontN)))
        tabla.AddCell(col4)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("DIRECCION:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Direccion").ToString(), fontN)))
        tabla.AddCell(col2)
        col3 = CrearCeldaPdf(New PdfPCell(New Phrase("TOTAL:", fontB)), BaseColor.LIGHT_GRAY)
        tabla.AddCell(col3)
        col4 = CrearCeldaPdf(New PdfPCell(New Phrase(FormatNumber(dtMatNotificaciones.Rows(0).Item("ImporteTotal").ToString(), 2), fontN)), BaseColor.LIGHT_GRAY)
        tabla.AddCell(col4)
        doc.Add(tabla)
        ...
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("ENVASES CON BILLETES:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("0", fontN)))
        tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("ENVASES CON MORRALLA:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("0", fontN)))
        tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("ENVASES MATERIALES:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Envases").ToString(), fontN)))
        tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("ENVASES TOTALES:", fontB)))
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("CantEnvases").ToString() + " +" + dtMatNotificaciones.Rows(0).Item("EnvasesSN").ToString() + "S/N", fontN)))
        tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("IMPORTE EN LETRAS:", fontB)), Nothing, 2)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(fn_EnLetras(dtMatNotificaciones.Rows(0).Item("ImporteTotal").ToString()), fontN)), Nothing, 6)
        tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("ENTREGAR ENVASES EN:", fontB)), Nothing, 2)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Nombre_Comercial").ToString() + "(" + dtMatNotificaciones.Rows(0).Item("Direccion").ToString() + ")", fontN)), Nothing, 6)
        tabla.AddCell(col2)
        'col1 = CrearCeldaPdf(New PdfPCell(New Phrase("DIRECCIÓN:", fontB)), Nothing, 2)
        'tabla.AddCell(col1)
        'col2 = CrearCeldaPdf(New PdfPCell(New Phrase(dtMatNotificaciones.Rows(0).Item("Direccion").ToString(), fontN)), Nothing, 6)
        'tabla.AddCell(col2)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("SELLOS:", fontB)), Nothing, 2)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(envasesN, fontN)), Nothing, 6)
        tabla.AddCell(col2)
        ...
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("FIRMA DE REMITENTE:", fontB)), Nothing, 0, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col1)

        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("TRANSPORTACION DE VALORES", fontB)), Nothing, 0, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)

        tabla.AddCell(cvacio)
        col1 = CrearCeldaPdf(New PdfPCell(iTextSharp.text.Image.GetInstance(QrMateriales(dtMatNotificaciones.Rows(0).Item("Nombre_em").ToString()))), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(iTextSharp.text.Image.GetInstance(QrMateriales(dtMatNotificaciones.Rows(0).Item("NombreTraslado").ToString()))), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)

        tabla.AddCell(cvacio)
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("FIRMA DE CONSIGNATORIO:", fontB)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("FECHA Y HORA DE SERVICIO:", fontB)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)

        tabla.AddCell(cvacio)
        col1 = CrearCeldaPdf(New PdfPCell(iTextSharp.text.Image.GetInstance(QrMateriales(dtMatNotificaciones.Rows(0).Item("Nombre_firma").ToString()))), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase(vbCrLf + " " + vbCrLf + dtMatNotificaciones.Rows(0).Item("Fechar_entrega").ToString() + vbCrLf + dtMatNotificaciones.Rows(0).Item("Horar_entrega").ToString(), fontN)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col2)
        tabla.AddCell(cvacio)
        doc.Add(tabla)
        ...
        col1 = CrearCeldaPdf(New PdfPCell(New Phrase("IMPORTANTE", fontB)), Nothing, Nothing, PdfPCell.ALIGN_CENTER)
        tabla.AddCell(col1)
        col2 = CrearCeldaPdf(New PdfPCell(New Phrase("° CUALQUIER ALTERACIÓN HACE NULO ESTE DOCUMENTO." + vbCrLf +
        "° LA COMPAÑIA  NO SERA RESPONSABLE POR INCUMPLIMIENTO DE ESTE SERVICIO EN CASOS FORTUITOS O FUERZA MAYOR." + vbCrLf +
        "° LA COMPAÑIA  NO ATENDERA RECLAMACION ALGUNA DESPUES DE 60 DIAS DE LA FECHA DE ESTE UNICO DOCUMENTO." + vbCrLf +
        "° NO ENTREGUE SUS VALORES SI EXISTE DUDA SOBRE LA IDENTIDAD DEL PERSONAL." + vbCrLf +
        "° PARA EFECTOS DE FACTURACION, CADA REMISION REPRESENTA UN SERVICIO.", fontN)))
        ...
        doc.Add(tabla)
        'RectanguloMateriales(29.0F, 70.0F, 538.0F, 568.0F)
        doc.Close()
